...
//console.log(sceneEl.querySelector('a-box'));
  ...
  AFRAME.registerComponent('do-something-once-loaded', {
    ...
    curr.object3D.position.set(-5*pos, 0, -35*(this.data.mydata[0]));
  ...
  AFRAME.registerComponent('do-something-once', {
    ...
    this.material = new THREE.MeshStandardMaterial({color: data.color});
    this.mesh = new THREE.Mesh(this.geometry, this.material);
    lft.object3D.position.set(-5*pos, 0, -35*(this.data.mydata[0]));
    //entityEl.object3D.position.z += (3*-35);
 ...
 // for(var i=1; i<=3; ++i){
  ...
  curr.setAttribute('do-something-once-loaded',{mydata:[1,1]});
  sceneEl.appendChild(curr);
  var lft = document.createElement('a-box');
  lft.setAttribute('do-something-once',{mydata:[1,-1]});
